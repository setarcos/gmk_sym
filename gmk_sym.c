...
/*            quit when this happens, give Fatal error messsage. (Chris  */
/*              Ellec)                                                   */
...
/* puon: added "cc" */
;;
device name, name, name location(tl, tc, tr, bl, bc, br, cc), X size in pins, Y size in pins 7474, 74 HC74, tr, 3, 5;;
;;
pin name, pin number, shape(line, clock, dot), side(r, l, t, b), pin position
    D, 2, line, L, 1 CLK, 3, clock, L, 4 Q, 5, line, R, 1 / Q, 6, dot, R, 4 CLR, 4, dot, T, 1 PRE, 1, dot, B, 1
/****************************************************/
#endif
#ifdef HAVE_CONFIG_H
#include "config.h"
...
void pin_add(int pos_x, int pos_y, char *pin, int shape, int dir, char *name, char *type)
{
	...
	if (shape == LINE_SHAPE) {
		...
		printf("P %d %d %d %d %d 0 1\n", pos_x - 100 * xdir, pos_y - 100 * ydir, pos_x - pin_len * xdir, pos_y - pin_len * ydir, WHITE);
		printf("{\n");
	} else if (shape == CLOCK_SHAPE) {
		printf("L %d %d %d %d %d 0 0 0 -1 -1\n", pos_x - 100 * ydir, pos_y - 100 * xdir, pos_x + 100 * xdir, pos_y + 100 * ydir, GREEN);
		printf("L %d %d %d %d %d 0 0 0 -1 -1\n", pos_x + 100 * ydir, pos_y + 100 * xdir, pos_x + 100 * xdir, pos_y + 100 * ydir, GREEN);
		printf("P %d %d %d %d %d 0 1\n", pos_x, pos_y, pos_x - pin_len * xdir, pos_y - pin_len * ydir, WHITE);
		printf("{\n");
	}
